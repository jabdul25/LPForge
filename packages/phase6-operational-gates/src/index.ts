...
import {loadMainnetCanaryDeploymentPolicyFile} from '../../canary/src/index.js';
...
export function loadPhase6LiveAuthorizationConfig(env:NodeJS.ProcessEnv=process.env):Phase6LiveAuthorizationConfig{let executionPolicyLoaded=false,executionPolicyEnabled=false;try{const policy=loadMainnetCanaryDeploymentPolicyFile(env.LPFORGE_EXECUTION_POLICY_PATH?.trim()||'policies/live-execution-policy.json');executionPolicyLoaded=true;executionPolicyEnabled=policy.status==='ENABLED';}catch{}const signerBackendConfigured=Boolean(env.LPFORGE_P6_SIGNER_BACKEND_ID?.trim());return{liveSigning:yes(env.LIVE_SIGNING),liveExecution:yes(env.LPFORGE_LIVE_EXECUTION),mainnetCanary:yes(env.LPFORGE_MAINNET_CANARY),...(env.LPFORGE_P6_OPERATOR_APPROVAL_ID?.trim()?{operatorApprovalId:env.LPFORGE_P6_OPERATOR_APPROVAL_ID.trim()}:{}),...(env.LPFORGE_P6_OPERATOR_APPROVED_BY?.trim()?{operatorApprovedBy:env.LPFORGE_P6_OPERATOR_APPROVED_BY.trim()}:{}),...(env.LPFORGE_P6_OPERATOR_APPROVAL_EXPIRES_AT?.trim()?{operatorApprovalExpiresAt:env.LPFORGE_P6_OPERATOR_APPROVAL_EXPIRES_AT.trim()}:{}),signerBackendConfigured,remoteSignerConfigured:signerBackendConfigured&&Boolean(env.LPFORGE_P6_SIGNER_PUBLIC_KEY?.trim())&&Boolean(env.LPFORGE_P6_REMOTE_SIGNER_URL?.trim())&&Boolean(env.LPFORGE_P6_REMOTE_SIGNER_AUTH_TOKEN?.trim()),privateWriteRpcConfigured:Boolean(env.LPFORGE_P6_PRIVATE_WRITE_RPC_URL?.trim()),executionPolicyLoaded,executionPolicyEnabled};}
export function evaluatePhase6LivePathAuthorization(c:Phase6LiveAuthorizationConfig,now=new Date().toISOString()):Phase6LivePathStatus{const r:string[]=[];let blocked:Phase6LivePathStage|undefined;const block=(stage:Phase6LivePathStage,code:string)=>{if(!blocked)blocked=stage;r.push(code)};
 ...
 if(!c.remoteSignerConfigured)block('SIGN','P6_LIVE_REMOTE_SIGNER_NOT_CONFIGURED');
