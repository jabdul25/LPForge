...
export function loadPhase6LiveAuthorizationConfig(env:NodeJS.ProcessEnv=process.env):Phase6LiveAuthorizationConfig{let executionPolicyLoaded=false,executionPolicyEnabled=false;try{const policy=loadMainnetCanaryDeploymentPolicyFile(env.LPFORGE_EXECUTION_POLICY_PATH?.trim()||'policies/live-execution-policy.json');executionPolicyLoaded=true;executionPolicyEnabled=policy.status==='ENABLED';}catch{}return{liveSigning:yes(env.LIVE_SIGNING),liveExecution:yes(env.LPFORGE_LIVE_EXECUTION),mainnetCanary:yes(env.LPFORGE_MAINNET_CANARY),...(env.LPFORGE_P6_OPERATOR_APPROVAL_ID?.trim()?{operatorApprovalId:env.LPFORGE_P6_OPERATOR_APPROVAL_ID.trim()}:{}),...(env.LPFORGE_P6_OPERATOR_APPROVED_BY?.trim()?{operatorApprovedBy:env.LPFORGE_P6_OPERATOR_APPROVED_BY.trim()}:{}),...(env.LPFORGE_P6_OPERATOR_APPROVAL_EXPIRES_AT?.trim()?{operatorApprovalExpiresAt:env.LPFORGE_P6_OPERATOR_APPROVAL_EXPIRES_AT.trim()}:{}),signerBackendConfigured:Boolean(env.LPFORGE_P6_SIGNER_BACKEND_ID?.trim()),privateWriteRpcConfigured:Boolean(env.LPFORGE_P6_PRIVATE_WRITE_RPC_URL?.trim()),executionPolicyLoaded,executionPolicyEnabled};}
