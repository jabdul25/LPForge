...
export async function collectActiveCandidateEvidence(input:{api:MeteoraDataApi;adapter:MeteoraReadAdapter;rpc:SolanaRpcClient;store:Phase1Store;observedAt:string;policy?:Partial<ActiveCandidateEvidencePolicy>}):Promise<{authority:'DISCOVERY_OBSERVATION_ONLY';observedAt:string;results:Array<{poolAddress:string;status:'PASS'|'DEGRADED';maturity?:HistoryMaturity;estimate?:EconomicEstimateDraft;error?:string}>}>{
 ...
 const results=await bounded(candidates,p.maxConcurrentPoolReads,async candidate=>{try{
   ...
   await input.store.insertPoolSnapshot(pool);await input.store.insertBins(bins);await input.store.insertDataApiPool(apiPool as Record<string,unknown>,input.observedAt);await input.store.insertOhlcv(candidate.poolAddress,'5m',ohlcv.data as unknown as Array<Record<string,unknown>>,input.observedAt,'METEORA_API');await input.store.insertFeeVolumeObservations({poolAddress:candidate.poolAddress,observedAt:input.observedAt,source:'METEORA_API_HISTORICAL_5M',rows:fees.data.map(row=>({bucketAt:iso(row.timestamp*1000),fees:n(row.fees),protocolFees:n(row.protocol_fees),volume:n(row.volume),payload:{timestamp:row.timestamp}}))});const currentBucketAt=iso(Math.floor(Date.parse(input.observedAt)/(5*60_000))*5*60_000);await input.store.insertFeeVolumeObservations({poolAddress:candidate.poolAddress,observedAt:input.observedAt,source:'METEORA_API_ROLLING_5M',rows:[{bucketAt:currentBucketAt,fees:n(apiPool.fees?.['5m']),protocolFees:n(apiPool.protocol_fees?.['5m']),volume:n(apiPool.volume?.['5m']),payload:{observedAt:input.observedAt,rollingWindow:'5m'}}]});
   ...
   const since=iso(Date.parse(input.observedAt)-p.historyWindowMs),history=await input.store.loadOperationalHistory(candidate.poolAddress,since,2000),maturity=await assessHistoryMaturity({poolAddress:candidate.poolAddress,assessedAt:input.observedAt,history,staleAfterMs:p.staleAfterMs}),persistedFeeEvidence=await input.store.loadFeeVolumeObservations(candidate.poolAddress,since,1000),feeBuckets=persistedFeeEvidence.map(row=>({timestamp:Math.floor(Date.parse(row.bucketAt)/1000),fees:row.fees,protocol_fees:row.protocolFees,volume:row.volume})),estimate=await deriveEventPathEconomicEstimate({poolAddress:candidate.poolAddress,asOf:input.observedAt,dataApiPool:apiPool,feeBuckets,history});
