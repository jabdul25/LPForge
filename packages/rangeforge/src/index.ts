...
export function generateRangeUniverse(input:{activeBinId:number;binStep:number;horizonMinutes:number;context:MarketContextSnapshot;structure:StructureFeatureVector;regime:RegimeAssessment;maxWidthBins?:number;minWidthBins?:number;enforceRequiredWidth?:boolean;}):RangeUniverse{
 ...
 const h15BoundedDisplacement=Number.isFinite(h15.maxAnchorDisplacementBins)?h15.maxAnchorDisplacementBins:h15.absoluteBins,h1BoundedDisplacement=Number.isFinite(h1.maxAnchorDisplacementBins)?h1.maxAnchorDisplacementBins:h1.absoluteBins;
 const volatilityMove=Math.max(4,h15BoundedDisplacement,h1BoundedDisplacement*horizonScale*.75)*volMult*transitionMult;
 const h1NetMovement=Number.isFinite(h1.netBins)?Math.abs(h1.netBins):Math.abs(h1.binVelocityPerMinute)*input.horizonMinutes;
 const survivalMove=Math.max(4,h1NetMovement*.55)*volMult*transitionMult;
 ...
 const ordered=candidates.sort((a,b)=>a.widthBins-b.widthBins||a.family.localeCompare(b.family));return{activeBinId:input.activeBinId,binStep:input.binStep,horizonMinutes:input.horizonMinutes,candidates:ordered,movementBasisBins:movementBasis,volatilityMultiplier:volMult,minimumIncludedBins:minWidth,volatilityRequiredWidthBins,survivalHorizonRequiredWidthBins,finalMinimumWidthBins};
}
