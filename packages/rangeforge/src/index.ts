...
export function generateRangeUniverse(input:{activeBinId:number;binStep:number;horizonMinutes:number;context:MarketContextSnapshot;structure:StructureFeatureVector;regime:RegimeAssessment;maxWidthBins?:number;minWidthBins?:number;enforceRequiredWidth?:boolean;}):RangeUniverse{
 ...
 /* Required width answers displacement from the entry reference, never the
  * total path length.  Cumulative absolute travel remains useful telemetry,
  * but choppy reversals must not repeatedly widen a single LP envelope. */
 const finiteOr=(value:number|undefined,fallback:number)=>Number.isFinite(value)?Number(value):fallback;
 /* Older immutable replay fixtures predate maxAnchorDisplacementBins.  They
  * retain their historical travel value only as a compatibility fallback;
  * every current MarketContextSnapshot supplies the bounded field. */
 const h15Excursion=finiteOr(h15.maxAnchorDisplacementBins,finiteOr(Math.abs(h15.netBins),h15.absoluteBins)),h1Excursion=finiteOr(h1.maxAnchorDisplacementBins,finiteOr(Math.abs(h1.netBins),h1.absoluteBins));
 const volatilityMove=Math.max(4,h15Excursion,h1Excursion*horizonScale*.75)*volMult*transitionMult;
 /* Survival is deliberately directional/horizon based.  It does not reuse
  * absoluteBins or binVelocityPerMinute, which are cumulative-travel signals. */
 const survivalMove=Math.max(4,finiteOr(Math.abs(h15.netBins),finiteOr(h15.binVelocityPerMinute*15,h15.absoluteBins))*horizonScale,finiteOr(Math.abs(h1.netBins),finiteOr(h1.binVelocityPerMinute*60,h1.absoluteBins))*(input.horizonMinutes/60))*volMult*transitionMult;
 ...
 for(const [family,m] of families){/* Production family labels select liquidity shape/strategy below; they may not multiply a resolved safe span. */let width=explicitPolicyFloor?finalMinimumWidthBins:boundedOddWidth(movementBasis*m*2+1,minWidth,maxWidth);if(width>maxWidth){if(explicitPolicyFloor)throw new Error('RANGE_REQUIRED_WIDTH_EXCEEDS_MAXIMUM');width=maxWidth;}const half=explicitPolicyFloor?Math.floor((width-1)/2):Math.floor(width/2);const direction=input.structure.trendDirection;const skewStrength=clamp((input.structure.trendEfficiency*.4+Math.abs(input.context.horizons['15m'].returnPct)/10)*.6,0,.35);let lowerHalf=half,upperHalf=explicitPolicyFloor?width-1-half:half;if(direction>0){upperHalf=Math.round((explicitPolicyFloor?width-1-half:half)*(1+skewStrength));lowerHalf=Math.max(1,width-1-upperHalf);}else if(direction<0){lowerHalf=Math.round(half*(1+skewStrength));upperHalf=Math.max(1,width-1-lowerHalf);}width=lowerHalf+upperHalf+1;if(width>maxWidth){if(explicitPolicyFloor)throw new Error('RANGE_REQUIRED_WIDTH_EXCEEDS_MAXIMUM');const overflow=width-maxWidth;lowerHalf=Math.max(1,lowerHalf-Math.ceil(overflow/2));upperHalf=Math.max(1,upperHalf-Math.floor(overflow/2));width=lowerHalf+upperHalf+1;}if(explicitPolicyFloor&&width!==finalMinimumWidthBins)throw new Error('RANGE_FINAL_GEOMETRY_WIDTH_MISMATCH');
