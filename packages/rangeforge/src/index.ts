...
export function generateStrategyCandidates(input:{universe:RangeUniverse;strategies?:Phase3StrategyFamily[];orientations?:Phase3Orientation[];strategyOrientations?:Partial<Record<Phase3StrategyFamily,Phase3Orientation[]>>;capitalFractions?:number[]}):RangeStrategyCandidate[]{
 ...
 for(const g of input.universe.candidates)for(const strategy of strategies)for(const orientation of input.strategyOrientations?.[strategy]??orientations)for(const capitalFraction of fractions){if(!(capitalFraction>0&&capitalFraction<=1))throw new Error('LPFORGE_RANGE_CAPITAL_FRACTION');const perBinWeights=buildStrategyWeights(g,strategy,orientation);out.push({...g,id:`${g.id}-${strategy.toLowerCase()}-${orientation.toLowerCase()}-${Math.round(capitalFraction*1000)}`,strategy,orientation,capitalFraction,perBinWeights});}
 return out;
}
