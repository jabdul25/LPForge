...
import { applyFeeEvidenceCalibration } from '../../fee-evidence-calibration/src/index.js';
...
export interface ShadowRecommendationInput {
  ...
  orientations?:RangeStrategyCandidate['orientation'][];strategies?:RangeStrategyCandidate['strategy'][];strategyOrientations?:Partial<Record<RangeStrategyCandidate['strategy'],RangeStrategyCandidate['orientation'][]>>;capitalFractions?:number[];maxRangeWidthBins?:number;minRangeWidthBins?:number;qualificationPolicy?:Phase3QualificationPolicyId;
}
/**
...
export function prepareCandidateReplay(input:Pick<ShadowRecommendationInput,'historicalFrames'|'decisionAt'|'horizonMinutes'>,candidate:RangeStrategyCandidate):CandidateReplayPreparation{
 const frames=[...input.historicalFrames].filter(f=>Date.parse(f.observedAt)<=Date.parse(input.decisionAt)).sort((a,b)=>Date.parse(a.observedAt)-Date.parse(b.observedAt));const horizon=input.horizonMinutes*60000,maxTerminalSlack=450_000;let continuityInsufficient=false;
 for(let i=frames.length-1;i>=0;i--){const anchor=frames[i]!,anchorMs=Date.parse(anchor.observedAt),fixedReplayCandidate=rebaseCandidateForReplay(candidate,anchor.activeBinId);if(deriveSyntheticPositionShareRaw(anchor)<=0n||!candidateCoverage(fixedReplayCandidate,anchor))continue;const replay:BinFrame[]=[];for(const frame of frames.slice(i)){if(!candidateCoverage(fixedReplayCandidate,frame)){continuityInsufficient=true;break;}if(Date.parse(frame.observedAt)>anchorMs+horizon+maxTerminalSlack)break;replay.push(frame);}const span=Date.parse(replay.at(-1)?.observedAt??'')-anchorMs;if(replay.length&&span>=horizon&&span<=horizon+maxTerminalSlack)return{frames:replay};continuityInsufficient=true;}
...
export async function buildShadowRecommendation(input:ShadowRecommendationInput):Promise<ShadowRecommendation>{
 ...
 const configuredWidth=input.maxRangeWidthBins??100,configuredMinimum=input.minRangeWidthBins??11,configuredMinimumExplicit=input.minRangeWidthBins!==undefined;if(!Number.isInteger(configuredWidth)||configuredWidth<3||configuredWidth>1400||!Number.isInteger(configuredMinimum)||configuredMinimum<1||configuredMinimum>configuredWidth)throw new Error('LPFORGE_SHADOW_RANGE_WIDTH_POLICY_INVALID');const universe=generateRangeUniverse({activeBinId:input.activeBinId,binStep:input.binStep,horizonMinutes:input.horizonMinutes,context,structure,regime,maxWidthBins:configuredWidth,minWidthBins:configuredMinimum,enforceRequiredWidth:configuredMinimumExplicit});const generatedCandidates=generateStrategyCandidates({universe,strategies:input.strategies??['SPOT','CURVE','BID_ASK'],orientations:input.orientations??['BALANCED'],...(input.strategyOrientations?{strategyOrientations:input.strategyOrientations}:{}),capitalFractions:input.capitalFractions??[1]});const candidates=await Promise.all(generatedCandidates.map(withMeteoraSdkWeights));
 ...
 const rawSimulations=candidates.map(candidate=>{const preparation=prepareCandidateReplay(input,candidate);if(!preparation.frames)return replayEvidenceInsufficient(candidate,input.capitalValue,preparation.reason??'CANDIDATE_REPLAY_COVERAGE_INSUFFICIENT');const replay=preparation.frames;replayAnchorByCandidate.set(candidate.id,replay[0]!.observedAt);const replayEvents=input.historicalEvents.filter(event=>Date.parse(event.stamp.observedAt)>=Date.parse(replay[0]!.observedAt));return simulateCandidateEconomics({candidate,pool:input.pool,frames:replay,events:replayEvents,totalPositionShareRaw:input.totalPositionShareRaw??deriveSyntheticPositionShareRaw(replay[0]!),rawUnitValueX:input.rawUnitValueX,rawUnitValueY:input.rawUnitValueY,capitalValue:input.capitalValue,...(input.costs?{costs:input.costs}:{})});});
 // Only Candidate-Primary authority consumes calibrated copies. Global
 // opportunity dispersion remains raw replay, avoiding double risk penalty.
 const simulations=rawSimulations.map(applyFeeEvidenceCalibration);
 const outcomeDispersion=deriveOutcomeDispersion(rawSimulations),economics=estimateOpportunityEconomics({capitalValue:input.capitalValue,horizonMinutes:input.horizonMinutes,rates:input.rateEvidence,pool:input.poolAssessment,regime,structure,regimeHistory,outcomeDispersion});
 ...
 const hardReasonCodes=new Set(['CANDIDATE_REPLAY_COVERAGE_INSUFFICIENT','CANDIDATE_REPLAY_CONTINUITY_INSUFFICIENT','FEE_CALIBRATION_EVIDENCE_INSUFFICIENT','RANGE_SURVIVAL_EVIDENCE_INSUFFICIENT','RANK_EVIDENCE_NON_ACTIONABLE']);
