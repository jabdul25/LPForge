...
export interface ShadowRecommendationInput {
  ...
  orientations?:RangeStrategyCandidate['orientation'][];strategies?:RangeStrategyCandidate['strategy'][];capitalFractions?:number[];
}
...
export async function buildShadowRecommendation(input:ShadowRecommendationInput):Promise<ShadowRecommendation>{
 ...
 const universe=generateRangeUniverse({activeBinId:input.activeBinId,binStep:input.binStep,horizonMinutes:input.horizonMinutes,context,structure,regime,maxWidthBins:Math.min(1400,Math.max(31,Math.floor(input.historicalFrames[0]?.bins.length??1400)))});const candidates=generateStrategyCandidates({universe,strategies:input.strategies??['SPOT','CURVE','BID_ASK'],orientations:input.orientations??['BALANCED'],capitalFractions:input.capitalFractions??[1]});
