...
export function buildTransactionPlan(r:PlanRequest):TransactionPlan{
  ...
  if(r.action==='OPEN'){const funding=(r.metadata?.entryFunding??{}) as Record<string,unknown>,swapLamports=String(funding.solToPairedTokenLamports??'0');const needsSwap=/^\d+$/.test(swapLamports)&&BigInt(swapLamports)>0n;let sequence=1;if(needsSwap)transactions.push(tx('JUPITER_SWAP',sequence++,r.ownerAddress,undefined,{solToPairedTokenLamports:swapLamports,totalPairedTokenRaw:String(funding.totalPairedTokenRaw??''),provider:'JUPITER_METIS'}));transactions.push(tx('METEORA_OPEN',sequence,r.ownerAddress,r.replacementPositionAddress??r.positionAddress,{lowerBinId:r.lowerBinId,upperBinId:r.upperBinId,strategy:r.strategy}));}
