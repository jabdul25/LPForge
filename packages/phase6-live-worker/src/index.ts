...
import { assessLifecycleSettlement, toIsoTimestamp } from "../../db/src/index.js";
import type {
  AutonomousPlan,
  ...
  for (const plan of plans) {
    ...
    const adoptOpenPosition = async (): Promise<boolean> => {
      ...
      if(!construction.fullyConstructed){
        ...
        await input.store.upsertPartialEntryRecovery({planId:plan.planId,poolAddress:plan.poolAddress,ownerAddress:plan.ownerAddress,tokenMint:String(partial?.token_mint??funding.tokenMint??''),fundingTransactionId:String(partial?.funding_transaction_id??plan.steps[0]?.transactionId??'P6_PARTIAL_ENTRY'),fundingSignature:String(partial?.funding_signature??journal.signature??''),fundedAt:toIsoTimestamp(partial?.funded_at??input.now),pairedTokenAmount:String(partial?.paired_token_amount??funding.totalPairedTokenRaw??'0'),intendedCapitalLamports:capital,intendedRange:{lowerBinId:Number(positionTruth.lowerBinId),upperBinId:Number(positionTruth.upperBinId)},state:'RECONCILIATION_REQUIRED',walletTruth:{...(partial?.wallet_truth&&typeof partial.wallet_truth==='object'?partial.wallet_truth as Record<string,unknown>:{}),refreshRequired:true},payload:{partialEntry:true,positionAddress:address,reasonCodes:construction.reasonCodes},updatedAt:input.now});
        return false;
      }
      await input.store.upsertOwnedPosition({
        lpforgePositionId: `position-${address}`,
