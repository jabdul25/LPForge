...
export function deriveTransactionAssetEffects(receipt:ConfirmedExecutionReceipt,context:TransactionAssetEffectContext):TransactionAssetEffects{
  ...
  const supportedNativeOutput=residual>0n&&hasJupiter&&context.outputMint===WSOL_MINT&&base.unclassifiedNativeEffects.length===0&&base.unclassifiedTokenEffects.length===0;
  if(supportedNativeOutput){const effect:NativeAssetEffect={classification:"SWAP_OUTPUT",accountAddress:context.ownerAddress,accountIndex:ownerIndex,amountLamports:residual,direction:"IN",evidence:["owner-native-residual","jupiter-program","wsol-output-context"]};base.swapOutputEffects.push(effect);accountedNative+=residual;residual=0n;}
