...
function candidateFromThesis(thesis:NonNullable<ShadowRecommendation['thesis']>):RangeStrategyCandidate{
  const s=thesis.selectedCandidate; const width=Math.max(1,s.widthBins); const center=Math.floor((s.lowerBinId+s.upperBinId)/2); const family=(s.id.toUpperCase().includes('DEFENSIVE')?'DEFENSIVE':s.id.toUpperCase().includes('WIDE')?'WIDE':s.id.toUpperCase().includes('NARROW')?'NARROW':'BASE') as RangeStrategyCandidate['family'];
  ...
  return{...geometry,strategy,orientation:s.orientation as RangeStrategyCandidate['orientation'],capitalFraction:s.capitalFraction,perBinWeights:buildStrategyWeights(geometry,strategy)};
}
