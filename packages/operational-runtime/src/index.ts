...
export async function evaluateOperationalCycle(input:OperationalCycleInput):Promise<OperationalCycleResult>{
  ...
  const allocation=input.productionCapitalPolicy&&input.productionPoolCapital!==undefined
    ?allocateProductionCapital({walletCapital:input.walletCapital,policy:input.productionCapitalPolicy,requests:[{id:`alloc-${shadow.thesis.thesisId}`,pool:input.pool.address,token:input.dataApiPool.token_y?.address??input.pool.tokenYMint,requested:input.walletCapital*shadow.thesis.selectedCandidate.capitalFraction,maxPoolCapital:input.productionPoolCapital,entryReady:entry.decision==='ENTRY_READY',expectedNetValue:shadow.economics.expectedNetLpValue}]})
