...
export interface OperationalCycleInput {
  ...
  /** Inclusive policy floor, passed to RangeForge before P3/P4 economics. */
  minRangeWidthBins?:number;
...
export async function evaluateOperationalCycle(input:OperationalCycleInput):Promise<OperationalCycleResult>{
  ...
  const protectedFunding=await resolveProtectedPhase6ExactSolEntryFunding({strategy:candidate.strategy,orientation:candidate.orientation,capitalLamports,activeBinId:input.pool.activeBinId,binStep:input.pool.binStep,lowerBinId:candidate.lowerBinId,upperBinId:candidate.upperBinId,activeBinXAmount:activeBin.amountX,activeBinYAmount:activeBin.amountY,perBinWeights:candidate.perBinWeights,quoteForFunding:async(inputAmount,requiredOutputAmount)=>input.swapQuoteProvider?input.swapQuoteProvider.quote({inputMint:input.pool.tokenYMint,outputMint:input.pool.tokenXMint,inputAmount,requiredOutputAmount}):{status:'UNAVAILABLE',reasonCodes:['P6_SWAP_QUOTE_PROVIDER_REQUIRED']}});
  const entryFunding=protectedFunding.status==='APPROVED'?protectedFunding.funding:undefined,swapQuote=protectedFunding.swapQuote;
  const swapReady=protectedFunding.status==='APPROVED';let phase4Status:OperationalStatus=entry.decision==='ENTRY_READY'&&risk.decision==='APPROVE'&&allocation.totalAllocated>0&&swapReady?'ENTRY_READY':entry.decision==='ENTRY_READY'&&!swapReady?'WAIT':entry.decision;
  ...
  if(phase4Status==='ENTRY_READY'){
    ...
    else {plan=buildTransactionPlan({action:'OPEN',cluster:'mainnet-beta',ownerAddress:input.ownerAddress,poolAddress:input.pool.address,...(input.replacementPositionAddress?{replacementPositionAddress:input.replacementPositionAddress}:{}),thesisId:shadow.thesis.thesisId,candidateId:candidate.id,observedAt:input.observedAt,expiresAt,capitalLamports,lowerBinId:entryFunding!.lowerBinId,upperBinId:entryFunding!.upperBinId,activeBinId:input.pool.activeBinId,binStep:input.pool.binStep,strategy:entryFunding!.strategy,...(input.maxRangeWidthBins!==undefined?{maxPositionWidthBins:input.maxRangeWidthBins}:{}),metadata:{authority:'AUTONOMOUS_DISPATCH',operationalCompletion:true,positionSigner:input.replacementPositionAddress?'EXTERNAL_POSITION_ADDRESS':'EPHEMERAL_POSITION_V2',entryFunding:{orientation:entryFunding!.orientation,pairedTokenTargetBps:entryFunding!.pairedTokenTargetBps,solForLpLamports:entryFunding!.solForLpLamports.toString(),solToPairedTokenLamports:entryFunding!.solToPairedTokenLamports.toString(),totalPairedTokenRaw:entryFunding!.totalPairedTokenRaw.toString(),meteoraSdkVersion:entryFunding!.sdkVersion,reasonCodes:entryFunding!.reasonCodes,protectedFundingQuoteIterations:protectedFunding.quoteIterations,protectedFundingMinOutRaw:protectedFunding.swapQuote?.quote?.otherAmountThreshold?.toString()}}});phase5Status='PLAN_PREPARED_BUILD_ONLY';phase4Status='PLAN_PREPARED';reasonCodes.push('OPERATIONAL_P5_AUTONOMOUS_PLAN_READY');}
  }
  reasonCodes.push(...entry.reasonCodes,...risk.reasonCodes,...(protectedFunding.status==='APPROVED'?protectedFunding.funding.reasonCodes:protectedFunding.reasonCodes),...(swapQuote?.reasonCodes??[]));const core={...base,phase3Status:'ENTRY_READY' as const,phase4Status,phase5Status,shadow,entry,risk,allocation,...(entryFunding?{entryFunding}:{}),...(swapQuote?{swapQuote}:{}),...(plan?{plan}:{}),reasonCodes:[...new Set(reasonCodes)].sort()};return{cycleId:await sha256Hex(canonicalJson(core)),...core};
}
