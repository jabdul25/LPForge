...
export interface EntryRecommendation extends Phase4EntryRecommendationContract {
 policyId:string;
 ...
 hardBlocks:string[];
 waitReasons:string[];
 phase4EconomicUncertainty:number;
 phase4TimingConfidence:number;
 uncertaintyNoLongerBlocking:true;
 removedBlockerReason:'WAIT_ECONOMIC_UNCERTAINTY'|null;
}
...
export function evaluateEntry(input:{features:EntryTimingFeatureVector;economics:OpportunityEconomics;thesis:MachineReadableLpThesis;observedAt:string;expiresAt:string;policy?:EntryPolicy;}):EntryRecommendation{
 ...
 return{phase:'P4',paperOnly:true,liveSigning:false,decision,observedAt:input.observedAt,expiresAt:input.expiresAt,reasonCodes,confidence,policyId:p.id,readinessScore:readiness,thesisId:input.thesis.thesisId,expectedNetLpValue:primaryExpectedNetLpValue,hardBlocks:hard,waitReasons:wait,phase4EconomicUncertainty:e.uncertainty,phase4TimingConfidence:confidence,uncertaintyNoLongerBlocking,removedBlockerReason};
}
