...
/** Flat, shadow-only economics for one LP opportunity at explicitly supplied capital; this adapts existing candidate simulation and VC-2 feasibility facts and is not a model or decision authority. */ export type CandidateCapitalRiskStatus='HARD_SAFETY_BLOCK'|'SOFT_ECONOMIC_RISK'|'CLEAR'; export type CandidateCapitalConfidenceState='ACTIONABLE_EVIDENCE'|'LIMITED_EVIDENCE'; export interface CandidateCapitalFeasibilityView {status:string;proposedCapitalLamports:string;allocatedCapitalLamports?:string;bindingConstraint?:string;failureReason?:string;maximumFeasibleCapitalLamports?:string;} export interface CandidateCapitalSimulationView {candidateId:string;feeValue:number;inventoryChangeValue:number;totalCostValue:number;netValue:number;evidenceActionable:boolean;warnings:string[];} export interface CandidateCapitalEconomics {candidateId:string;userSelectedCapitalLamports:string;allocatedCapitalLamports?:string;feasible:boolean;infeasibleReason?:string;maxSafeCapitalLamports?:string;expectedFeePnlSol?:number;expectedInventoryEffectSol?:number;expectedCostsSol?:number;expectedNetPnlSol?:number;expectedNetReturnBps?:number;riskStatus:CandidateCapitalRiskStatus;confidenceState:CandidateCapitalConfidenceState;evidenceObservedAt:string;authority:'SHADOW_ONLY';} export interface CandidateCapitalEconomicsDecisionContext {feasibility:CandidateCapitalFeasibilityView;simulation?:CandidateCapitalSimulationView;evidenceObservedAt:string;softRiskReasons?:string[];} const LAMPORTS_PER_SOL=1_000_000_000n; function finiteEconomicValue(value:number,name:string):number{if(!Number.isFinite(value))throw new Error(`LPFORGE_CANDIDATE_CAPITAL_ECONOMICS_${name}_INVALID`);return value;} /** No capital clipping or optimization occurs here. VC-2 is the sole source of feasibility. Infeasible requests deliberately have no economic PnL. */ export function evaluateOpportunity(input:{candidateId:string;userSelectedCapitalLamports:bigint;decisionContext:CandidateCapitalEconomicsDecisionContext}):CandidateCapitalEconomics{if(!input.candidateId)throw new Error('LPFORGE_CANDIDATE_CAPITAL_ECONOMICS_CANDIDATE_REQUIRED');if(input.userSelectedCapitalLamports<=0n)throw new Error('LPFORGE_CANDIDATE_CAPITAL_ECONOMICS_CAPITAL_INVALID');const feasibility=input.decisionContext.feasibility;if(feasibility.proposedCapitalLamports!==input.userSelectedCapitalLamports.toString())throw new Error('LPFORGE_CANDIDATE_CAPITAL_ECONOMICS_FEASIBILITY_CAPITAL_MISMATCH');const feasible=feasibility.status==='FEASIBLE_PRICE_TAKING'||feasibility.status==='FEASIBLE_NONLINEAR';const confidenceState:CandidateCapitalConfidenceState=input.decisionContext.simulation?.evidenceActionable?'ACTIONABLE_EVIDENCE':'LIMITED_EVIDENCE';const softRiskReasons=input.decisionContext.softRiskReasons??[];const base={candidateId:input.candidateId,userSelectedCapitalLamports:input.userSelectedCapitalLamports.toString(),...(feasibility.allocatedCapitalLamports?{allocatedCapitalLamports:feasibility.allocatedCapitalLamports}:{}),...(feasibility.maximumFeasibleCapitalLamports?{maxSafeCapitalLamports:feasibility.maximumFeasibleCapitalLamports}:{}),confidenceState,evidenceObservedAt:input.decisionContext.evidenceObservedAt,authority:'SHADOW_ONLY' as const};if(!feasible)return{...base,feasible:false,infeasibleReason:feasibility.failureReason??feasibility.bindingConstraint??feasibility.status,riskStatus:'HARD_SAFETY_BLOCK'};const simulation=input.decisionContext.simulation;if(!simulation||simulation.candidateId!==input.candidateId)throw new Error('LPFORGE_CANDIDATE_CAPITAL_ECONOMICS_SIMULATION_REQUIRED');const expectedFeePnlSol=finiteEconomicValue(simulation.feeValue,'FEE');const expectedInventoryEffectSol=finiteEconomicValue(simulation.inventoryChangeValue,'INVENTORY');const expectedCostsSol=finiteEconomicValue(simulation.totalCostValue,'COST');const expectedNetPnlSol=expectedFeePnlSol+expectedInventoryEffectSol-expectedCostsSol;const suppliedCapitalSol=Number(input.userSelectedCapitalLamports)/Number(LAMPORTS_PER_SOL);if(!(suppliedCapitalSol>0)&&Number.isFinite(suppliedCapitalSol))throw new Error('LPFORGE_CANDIDATE_CAPITAL_ECONOMICS_CAPITAL_INVALID');return{...base,feasible:true,expectedFeePnlSol,expectedInventoryEffectSol,expectedCostsSol,expectedNetPnlSol,expectedNetReturnBps:expectedNetPnlSol/suppliedCapitalSol*10_000,riskStatus:softRiskReasons.length?'SOFT_ECONOMIC_RISK':'CLEAR'};}
/** Additional shadow-only facts separate canonical constructibility from the
 * existing strict VC-2 price-taking policy. They do not affect authority. */
export interface CandidateCapitalEconomics {
  mechanicallyConstructible?:boolean;
  mechanicalFailureReason?:string;
  currentPolicyFeasible?:boolean;
  currentPolicyStatus?:string;
  currentPolicyFailureReason?:string;
  resultingOwnershipBps?:number;
