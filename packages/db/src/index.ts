  ...
  return {
    ...
    async reconcileLiveEvidenceAdmission(v) {
      ...
      try {
        ...
        if(promoted.length)await tx.query(`UPDATE market.pool_discovery_registry SET current_state='ACTIVE_CANDIDATE',reason_codes=(reason_codes - 'LIVE_EVIDENCE_WAITING_FOR_CAPACITY') || '["LIVE_EVIDENCE_ADMITTED"]'::jsonb,payload=payload||jsonb_build_object('liveEvidenceAdmission','ADMITTED','liveEvidenceAdmissionAt',$2::text,'liveEvidenceLeaseStartedAt',$2::text,'liveEvidenceLeaseExpiresAt',$3::text,'liveEvidenceLeaseFailures',0,'liveEvidencePhase3ConsumptionState','NONE','liveEvidencePhase3ReadyAt',NULL,'liveEvidencePhase3ReadyEventPathAt',NULL,'postEvidenceEvaluationState','NONE') WHERE pool_address=ANY($1::text[])`,[promoted.map(x=>x.poolAddress),v.observedAt,promotedLeaseExpiresAt]);
    ...
    async recordPostEvidenceEvaluationOutcome(v) {
      ...
      await db.query(`UPDATE market.pool_discovery_registry SET current_state='QUALIFIED',reason_codes=(reason_codes-'LIVE_EVIDENCE_ADMITTED')||'[\"LIVE_EVIDENCE_LEASE_TERMINAL_PHASE3\"]'::jsonb,payload=payload||jsonb_build_object('liveEvidenceAdmission','WAITING','liveEvidenceLeaseReleaseReason','LIVE_EVIDENCE_LEASE_TERMINAL_PHASE3','liveEvidenceLeaseReleasedAt',$2::text,'liveEvidenceLeaseNextEligibleAt',$3::text,'liveEvidencePhase3ConsumptionState','COMPLETED','liveEvidencePhase3ConsumedAt',$2::text,'postEvidenceEvaluationState','COMPLETED','postEvidenceEvaluationCompletedAt',$2::text,'postEvidenceEvaluationClearReason',$4::text) WHERE pool_address=$1 AND current_state='ACTIVE_CANDIDATE' AND payload->>'liveEvidencePhase3ConsumptionState'='PENDING'`,[v.poolAddress,v.observedAt,nextEligibleAt,clearReason]);
...
return 'APPLIED';
    ...
    async loadLifecycleSettlementInput(positionAddress){
      ...
      const [cash,lots,tx,res]=await Promise.all([
        ...
        db.query("SELECT s.transaction_id,a.signature,COALESCE(CASE WHEN a.state='EXPIRED' THEN 'EXPIRED' ELSE c.status END,a.state,CASE WHEN s.state IN ('CONFIRMED','COMPLETED') THEN 'CONFIRMED' ELSE s.state END) state FROM execution.lifecycle_plan_links link JOIN execution.transaction_steps s ON s.plan_id=link.plan_id LEFT JOIN LATERAL (SELECT attempt_id,signature,state FROM execution.submission_attempts WHERE transaction_id=s.transaction_id ORDER BY attempt DESC LIMIT 1) a ON true LEFT JOIN LATERAL (SELECT status FROM execution.confirmations WHERE attempt_id=a.attempt_id ORDER BY observed_at DESC LIMIT 1) c ON true WHERE link.lifecycle_id=$1 ORDER BY s.sequence",[l.lifecycle_id]),
    ...
    async persistLifecycleSolSettlement(v){
      ...
      try{
        ...
        await db.query("UPDATE execution.owned_positions SET lifecycle_state='SOL_SETTLED',reconciliation_status='MATCH' WHERE position_address=$1",[input.lifecycle.positionAddress]);
    ...
    async createLiveSolSettledLearningOutcome(v){
      ...
      const closeFee=feeAttribution.rows[0] as Record<string,unknown>|undefined,claimedFees=flows.rows.filter(flow=>String(flow.flow_type)==='FEE_CLAIM').reduce((total,flow)=>total+amount(flow),0n),rewardFees=flows.rows.filter(flow=>String(flow.flow_type)==='REWARD_CLAIM').reduce((total,flow)=>total+amount(flow),0n),embeddedCloseFees=closeFee?.attribution_status==='COMPLETE'&&closeFee.realized_lp_fee_value_lamports!==null&&closeFee.realized_lp_fee_value_lamports!==undefined?BigInt(String(closeFee.realized_lp_fee_value_lamports)):0n,directFees=claimedFees+embeddedCloseFees,txCosts=flows.rows.filter(flow=>['TX_COST','SWAP_COST'].includes(String(flow.flow_type))).reduce((total,flow)=>total+amount(flow),0n),management=plans.rows.filter(plan=>String(plan.role)!=='ENTRY'),counts=(action:string)=>management.filter(plan=>String(plan.action)===action).length,rec=(row.recommendation_payload??{}) as Record<string,unknown>,regime=(rec.regime??{}) as Record<string,unknown>,exit=(exitState.rows[0]?.payload??{}) as Record<string,unknown>,entry=Date.parse(String(row.entry_at)),settled=Date.parse(String(row.settled_at)),capital=BigInt(String(row.initial_capital_lamports)),pnl=BigInt(String(row.realized_sol_pnl_lamports)),outcomeId=`live-outcome:${row.settlement_id}`,returnFraction=capital>0n?Number(pnl)/Number(capital):undefined;
