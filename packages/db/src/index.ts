  ...
  return {
    ...
    async reconcileLiveEvidenceAdmission(v) {
      ...
      try {
        ...
        for(const waiting of waitingCandidates)await tx.query(`UPDATE market.pool_discovery_registry SET payload=payload||jsonb_build_object('liveEvidenceWaitingAt',COALESCE(payload->>'liveEvidenceWaitingAt',$2::text),'liveEvidenceTimesSkipped',COALESCE((payload->>'liveEvidenceTimesSkipped')::int,0)+1,'liveEvidenceLastPriority',$3) WHERE pool_address=$1`,[waiting.poolAddress,v.observedAt,Number(waiting.evidencePriority??waiting.priorityScore)]);
