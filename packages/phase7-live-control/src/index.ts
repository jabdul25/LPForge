...
export function buildPhase7LiveControlDecision(input:{authority:Phase7Authority;health:Phase7HealthAssessment;drift:Phase7DriftAssessment;incidents:Phase7Incident[]}):Phase7LiveControlDecision{
  const safety=derivePhase7SafetyState({health:input.health,incidents:input.incidents});const allowed=input.authority.mode!=='OBSERVE_ONLY'&&input.health.newEntriesAllowed&&input.drift.newEntriesAllowed&&!safety.entriesPaused;const daemonPlan=allowed?'DECISION_CYCLE':'OBSERVE_ONLY';const reasons=[...new Set([...input.authority.reasonCodes,...input.health.reasonCodes,...input.drift.reasonCodes,...safety.reasonCodes,...(!input.drift.newEntriesAllowed?['P7_CONTROL_DRIFT_BLOCK']:[]),...(safety.entriesPaused?['P7_CONTROL_SAFETY_PAUSE']:[]),...(input.authority.mode==='OBSERVE_ONLY'?['P7_CONTROL_OBSERVE_AUTHORITY']:[])])].sort();return{authorityMode:input.authority.mode,healthStatus:input.health.status,driftStatus:input.drift.status,safety,daemonPlan,newEconomicActionAllowed:allowed,reasonCodes:reasons,directSigner:false,directTransactionSend:false};
}
