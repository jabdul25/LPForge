...
export function verifyPhase7RuntimeReleaseEvidence(value:Phase7RuntimeReleaseEvidence,expectedSourceCommit?:string):{valid:true;reasonCodes:string[]} {
  ...
  if(!value.migrations?.passed||value.migrations.count<28||!String(value.migrations.latest).startsWith('M0028_')||!value.migrations.freshPostgres||!value.migrations.upgradePostgres)reasons.push('P7_RELEASE_EVIDENCE_MIGRATIONS');
...
export async function registerPhase7RuntimeReleaseEvidence(input:{store:Pick<Phase1Store,'insertPhase7StageEvidence'>;evidence:Phase7RuntimeReleaseEvidence;expectedSourceCommit?:string}){
  verifyPhase7RuntimeReleaseEvidence(input.evidence,input.expectedSourceCommit);const evidenceId=`P7-R10:${input.evidence.sourceCommit}`;await input.store.insertPhase7StageEvidence({evidenceId,stage:'P7-R10',status:'PASS',observedAt:input.evidence.generatedAt,evidenceHash:input.evidence.evidenceHash,payload:input.evidence as unknown as Record<string,unknown>});return{evidenceId,stage:'P7-R10' as const,status:'PASS' as const,sourceCommit:input.evidence.sourceCommit,evidenceHash:input.evidence.evidenceHash,productionAuthorityIssued:false as const,automaticPolicyPromotion:false as const};
}
