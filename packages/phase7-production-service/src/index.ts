...
export function productionPoolAddressesFromPolicy(policy:Pick<MainnetCanaryDeploymentPolicy,'pools'>){return[...new Set(policy.pools.map(pool=>pool.address))];}
/** The enabled execution policy is the sole source of the production universe. */
export function productionPoolAddresses(env:NodeJS.ProcessEnv){const path=env.LPFORGE_EXECUTION_POLICY_PATH?.trim()||'policies/live-execution-policy.json';return productionPoolAddressesFromPolicy(loadDeploymentPolicyFile(path));}
/** Discovery is advisory. Its candidates may be observed by the operator only when explicitly enabled, and never modify execution policy. */
/**
 * Static policy pools are evaluated every cycle.  Tier-A discovery candidates
 * are evaluated in a bounded round-robin so a growing candidate set cannot
 * turn the Phase-7 health probe into an RPC burst.
 */
...
export async function productionEvaluationPoolAddresses(store:Pick<Phase1Store,'listDiscoveryCandidates'>,env:NodeJS.ProcessEnv,rotationKey='',ownedPoolAddresses:readonly string[]=[]){const manual=productionPoolAddresses(env),owned=[...new Set(ownedPoolAddresses.filter(Boolean))];if((env.LPFORGE_DISCOVERY_OPERATOR_ENABLED??'false').toLowerCase()!=='true')return[...new Set([...manual,...owned])];const candidates=await store.listDiscoveryCandidates(['A']),observedAt=new Date().toISOString(),ready=candidates.filter(candidate=>isPhase3ReadyProductionEvaluationCandidate(candidate,observedAt)).sort((a,b)=>readyAt(a)-readyAt(b)||a.poolAddress.localeCompare(b.poolAddress)),completed=candidates.filter(candidate=>isPostEvidenceProductionEvaluationCandidate(candidate,observedAt)).sort((a,b)=>Date.parse(String(a.payload.postEvidenceEvaluationEligibleAt??''))-Date.parse(String(b.payload.postEvidenceEvaluationEligibleAt??''))||a.poolAddress.localeCompare(b.poolAddress)),active=candidates.filter(candidate=>candidate.state==='ACTIVE_CANDIDATE'&&!isPhase3ReadyProductionEvaluationCandidate(candidate,observedAt)),max=Math.max(1,Math.min(10,Number(env.LPFORGE_PRODUCTION_OPERATOR_MAX_POOLS??10)));if(ready.length+completed.length+active.length===0)return[...new Set([...manual,...owned])];const rotation=[...rotationKey].reduce((sum,char)=>(sum*31+char.charCodeAt(0))>>>0,0)%Math.max(1,active.length),rotatedActive=Array.from({length:Math.min(Math.max(0,max-ready.length-completed.length),active.length)},(_,index)=>active[(rotation+index)%active.length]!),selected=[...ready.slice(0,max),...completed.slice(0,Math.max(0,max-ready.length)),...rotatedActive].slice(0,max);return[...new Set([...manual,...owned,...selected.map(x=>x.poolAddress)])];}
 ...
 */
export async function runProductionGlobalSelectionCycle(input:{store:Pick<Phase1Store,'loadProductionGlobalCandidateFacts'|'loadProductionPoolSettlementHistory'|'insertProductionGlobalSelection'>;cwd:string;env:NodeJS.ProcessEnv;cycleKey:string;eligiblePoolAddresses:string[];sourceCommit?:string;buildId?:string}){
  const startedAt=new Date().toISOString(),ordered=fairProductionPoolOrder(input.eligiblePoolAddresses,input.cycleKey);
  ...
  const candidates:PoolCandidate[]=ordered.map(poolAddress=>{
    const row=byPool.get(poolAddress),thesis=record(row?.thesis),selected=record(thesis.selectedCandidate),economics=record(thesis.expectedEconomics),cycleEconomics=record(row?.economics),entry=record(row?.entry_payload),diagnostics=record(entry.diagnostics),history=deriveProductionPoolHistory({poolAddress,asOf:decisionCutoff,outcomes});
    const raw:Omit<PoolCandidate,'state'|'reasonCodes'>={poolAddress,...(row?.recommendation_id?{recommendationId:String(row.recommendation_id)}:{}),...(row?.thesis_id?{thesisId:String(row.thesis_id)}:{}),...(row?.selected_candidate_id?{candidateId:String(row.selected_candidate_id)}:{}),...(selected.strategy?{strategy:String(selected.strategy)}:{}),...(selected.orientation?{orientation:String(selected.orientation)}:{}),...(finite(selected.lowerBinId)===undefined?{}:{lowerBinId:finite(selected.lowerBinId)!}),...(finite(selected.upperBinId)===undefined?{}:{upperBinId:finite(selected.upperBinId)!}),...(finite(thesis.activeBinId)===undefined?{}:{activeBinId:finite(thesis.activeBinId)!}),...(row?.decision_at?{decisionAt:new Date(String(row.decision_at)).toISOString()}:{}),...(row?.expires_at?{expiresAt:new Date(String(row.expires_at)).toISOString()}:{}),phase3State:String(row?.phase3_state??'NO_VALID_CANDIDATE'),...(row?.phase4_state?{phase4State:String(row.phase4_state)}:{}),...(finite(economics.capitalValue)===undefined?finite(cycleEconomics.capitalValue)===undefined?{}:{capitalValue:finite(cycleEconomics.capitalValue)!}:{capitalValue:finite(economics.capitalValue)!}),...(finite(thesis.horizonMinutes)===undefined?finite(cycleEconomics.horizonMinutes)===undefined?{}:{horizonMinutes:finite(cycleEconomics.horizonMinutes)!}:{horizonMinutes:finite(thesis.horizonMinutes)!}),...(finite(economics.riskAdjustedExpectedNetEV)===undefined?{}:{riskAdjustedExpectedNetEv:finite(economics.riskAdjustedExpectedNetEV)!}),...(finite(economics.candidateExpectedFees)===undefined?{}:{predictedFees:finite(economics.candidateExpectedFees)!}),...(finite(economics.candidateExpectedInventoryPnl)===undefined?{}:{predictedInventoryPnl:finite(economics.candidateExpectedInventoryPnl)!}),...(finite(economics.uncertainty)===undefined?{}:{uncertainty:finite(economics.uncertainty)!}),...(finite(row?.confidence)===undefined?{}:{confidence:finite(row?.confidence)!}),...(finite(diagnostics.immediateOorRisk)===undefined?{}:{oorRisk:finite(diagnostics.immediateOorRisk)!}),...(thesis.provenance&&record(thesis.provenance).qualificationPolicy?{rankingPolicyId:String(record(thesis.provenance).qualificationPolicy)}:{}),history};
...
export async function runPhase7ProductionOnce(input:{cfg:Phase1Config;store:Phase1Store;runtimeId:string;instanceId:string;cycleKey:string;driftBaseline:Phase7EvaluationMetrics;sourceCommit?:string;policyHash?:string;cwd:string;env:NodeJS.ProcessEnv;leaseTtlMs?:number;restarted:boolean}):Promise<Phase7ProductionOnceResult>{
  ...
  // Snapshot the dynamic Tier-A universe once. Discovery may mutate it while
  // this cycle runs; health, drift, persisted control and operator probes must
  // all describe the same immutable target set.
  const evaluationPoolAddresses=await productionEvaluationPoolAddresses(input.store,input.env,input.cycleKey,[...openPools]),driftPoolAddresses=[...new Set([input.cfg.smokePoolAddress,...evaluationPoolAddresses])];
  ...
  try{
    globalSelection=await runProductionGlobalSelectionCycle({store:input.store,cwd:input.cwd,env:{...controlEnv,LPFORGE_P7_PLAN_DISPATCH_ENABLED:'false'},cycleKey:input.cycleKey,eligiblePoolAddresses:evaluationPoolAddresses,...(input.sourceCommit?{sourceCommit:input.sourceCommit}:{}),...(input.env.LPFORGE_BUILD_ID?{buildId:input.env.LPFORGE_BUILD_ID}:{})});
    const probes=[...globalSelection.probes];
    if(probes.length!==evaluationPoolAddresses.length||probes.some(probe=>!probe.operationalCycleComplete))throw new Error('LPFORGE_P7_GLOBAL_OPERATOR_CYCLE_INCOMPLETE');
