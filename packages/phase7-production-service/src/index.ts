...
/** A read-only fast probe; it cannot persist a plan or send a transaction. */
export interface Phase7PositionHealthProbe {observedPositions:number;availableLiveMarks:number;monitoringTier:'NORMAL'|'DANGER'|'CRITICAL';numericStopObserved:boolean;emergencyStopObserved:boolean;outputBytes:number;}
...
/**
 * A deliberately narrow, pre-authorized policy-bound bridge.  It does not
 ...
 */
export function resolveControlledCanaryWatch(input:{env:NodeJS.ProcessEnv;policy:MainnetCanaryDeploymentPolicy;now:string;authorization?:ControlledCanaryWatchAuthorization;portfolio:ControlledCanaryPortfolioFacts;decisionObservedAt?:string}):ControlledCanaryWatchDecision{
  ...
  if(!canary)reasons.push('P7_CONTROLLED_CANARY_POLICY_REQUIRED');
  ...
  return{enabled:true,activate:true,reasonCodes:['P7_CONTROLLED_CANARY_FRESH_PHASE4_AUTHORIZATION'],authorization:a!,approval:{approvalId:`canary-watch-${a!.entryEvaluationId}`,action:'PROMOTE_PRODUCTION',operatorId,issuedAt:input.now,expiresAt:approvalExpiry,reason:`Controlled policy-bound live canary: maxPositions=${canary!.maxConcurrentPositions}, capitalLamports=${canary!.exactLiquidityCapitalLamports}`}};
}
...
export async function runProductionGlobalSelectionCycle(input:{store:Pick<Phase1Store,'loadProductionGlobalCandidateFacts'|'loadProductionPoolSettlementHistory'|'insertProductionGlobalSelection'>;cwd:string;env:NodeJS.ProcessEnv;cycleKey:string;eligiblePoolAddresses:string[];selectionAdmissionSnapshots?:ProductionSelectionAdmissionSnapshot[];sourceCommit?:string;buildId?:string;probeRunner?:typeof runAutonomousDecisionProbe;nowImpl?:()=>number}){
  ...
  // environment default.  A missing policy fails closed at classification.
  const reentryPolicy=loadDeploymentPolicyFile(resolveLiveExecutionPolicyPath(input.env)).poolReentry;
  ...
  const candidates:PoolCandidate[]=ordered.map(poolAddress=>{
    const row=byPool.get(poolAddress),history=deriveProductionPoolHistory({poolAddress,asOf:decisionCutoff,outcomes}),operationalState=String(row?.operational_state??'REJECTED'),operationalPayload=row?.payload&&typeof row.payload==='object'&&!Array.isArray(row.payload)?row.payload as Record<string,unknown>:{},allocation=operationalPayload.allocation&&typeof operationalPayload.allocation==='object'&&!Array.isArray(operationalPayload.allocation)?operationalPayload.allocation as Record<string,unknown>:{},allocationRows=Array.isArray(allocation.allocations)?allocation.allocations:[],allocationForPool=allocationRows.find(value=>value&&typeof value==='object'&&String((value as Record<string,unknown>).pool??'')===poolAddress) as Record<string,unknown>|undefined,phase4State=String(row?.phase4_state??''),entryDecision=String(operationalPayload.entryDecision??'');
    const raw:Omit<PoolCandidate,'state'|'reasonCodes'>={poolAddress,operationalState:operationalState==='ENTRY_READY'||operationalState==='NO_TRADE'||operationalState==='WARMING'||operationalState==='REJECTED'?operationalState:'REJECTED',operationalReasonCodes:strings(row?.reason_codes),...(row?.recommendation_id?{recommendationId:String(row.recommendation_id)}:{}),...(row?.thesis_id?{thesisId:String(row.thesis_id)}:{}),...(row?.candidate_id?{candidateId:String(row.candidate_id)}:{}),...(row?.strategy?{strategy:String(row.strategy)}:{}),...(row?.orientation?{orientation:String(row.orientation)}:{}),...(finite(row?.lower_bin_id)===undefined?{}:{lowerBinId:finite(row?.lower_bin_id)!}),...(finite(row?.upper_bin_id)===undefined?{}:{upperBinId:finite(row?.upper_bin_id)!}),...(finite(row?.active_bin_id)===undefined?{}:{activeBinId:finite(row?.active_bin_id)!}),...(row?.observed_at?{decisionAt:new Date(String(row.observed_at)).toISOString()}:{}),phase3State:operationalState,...(phase4State?{phase4State}:{}),operationalEntryReady:phase4State==='ENTRY_READY'&&entryDecision==='ENTRY_READY',...(finite(allocationForPool?.allocated)===undefined?{}:{operationalCapitalAllocated:finite(allocationForPool?.allocated)!}),...(finite(row?.capital_value)===undefined?{}:{capitalValue:finite(row?.capital_value)!}),...(finite(row?.horizon_minutes)===undefined?{}:{horizonMinutes:finite(row?.horizon_minutes)!}),...(finite(row?.risk_adjusted_expected_net_ev)===undefined?{}:{riskAdjustedExpectedNetEv:finite(row?.risk_adjusted_expected_net_ev)!}),...(finite(row?.predicted_gross_fees)===undefined?{}:{predictedFees:finite(row?.predicted_gross_fees)!}),...(finite(row?.predicted_inventory_pnl)===undefined?{}:{predictedInventoryPnl:finite(row?.predicted_inventory_pnl)!}),...(finite(row?.uncertainty)===undefined?{}:{uncertainty:finite(row?.uncertainty)!}),...(finite(row?.confidence)===undefined?{}:{confidence:finite(row?.confidence)!}),...(finite(row?.oor_risk)===undefined?{}:{oorRisk:finite(row?.oor_risk)!}),history};
    return classifyProductionPoolCandidate({candidate:raw,cycleStartedAt:startedAt,decisionCutoff,outcomes,...(reentryPolicy?{reentryPolicy}:{})});
  });
...
/**
 * Runs only between normal P7 cycles while the preceding probe found a danger
 * tier. The child has read-only credentials and returns a compact result; it
 * does not append an observation stream or create an action.
 */
export async function runAutonomousPositionHealthProbe(input:{cwd:string;env:NodeJS.ProcessEnv}):Promise<Phase7PositionHealthProbe>{
  const childEnv:{[key:string]:string|undefined}={...input.env,LIVE_SIGNING:'false',LPFORGE_LIVE_EXECUTION:'false',LPFORGE_MAINNET_CANARY:'false',LPFORGE_DATA_MODE:'LIVE_READ_ONLY',LPFORGE_OPERATOR_MACHINE_SUMMARY:'true'};
  delete childEnv.LPFORGE_PREPARE_POSITION_ADDRESS;delete childEnv.LPFORGE_P6_PRIVATE_KEY;delete childEnv.LPFORGE_P6_PRIVATE_WRITE_RPC_URL;delete childEnv.LPFORGE_P6_SIGNER_BACKEND_ID;delete childEnv.LPFORGE_P6_SIGNER_MODE;delete childEnv.LPFORGE_P6_SIGNER_PUBLIC_KEY;delete childEnv.LIVE_SUBMISSION;delete childEnv.LPFORGE_P6_EXECUTION_RUNNER_ENABLED;
  const timeoutMs=Math.max(5_000,Math.min(60_000,Number(input.env.LPFORGE_P7_POSITION_HEALTH_PROBE_TIMEOUT_MS??20_000)));
  return new Promise((resolve,reject)=>{const child=spawn('node',['--enable-source-maps','.build/apps/operator/src/main.js','position-health-probe'],{cwd:input.cwd,env:childEnv,stdio:['ignore','pipe','pipe']});let out='',err='',settled=false;const timeout=setTimeout(()=>{if(settled)return;settled=true;(child as unknown as {kill:(signal:string)=>boolean}).kill('SIGTERM');reject(new Error(`LPFORGE_P7_POSITION_HEALTH_PROBE_TIMEOUT:${timeoutMs}:${redactProbeFailureDetail(err||out)}`));},timeoutMs);const done=(fn:()=>void)=>{if(settled)return;settled=true;clearTimeout(timeout);fn();};child.stdout?.on('data',x=>{out+=String(x);});child.stderr?.on('data',x=>{err+=String(x);});child.on('error',error=>done(()=>reject(error)));child.on('close',code=>done(()=>{const combined=out+'\n'+err;if((code??1)!==0)return reject(new Error(`LPFORGE_P7_POSITION_HEALTH_PROBE_FAILED:${code??1}:${redactProbeFailureDetail(err||out)}`));const line=combined.split(/\r?\n/).find(value=>value.includes('"event": "lpforge_position_health_probe"')||value.includes('"event":"lpforge_position_health_probe"'));if(!line)return reject(new Error('LPFORGE_P7_POSITION_HEALTH_PROBE_OUTPUT_MISSING'));try{const value=JSON.parse(line) as Record<string,unknown>,tier=value.monitoringTier; if(tier!=="NORMAL"&&tier!=="DANGER"&&tier!=="CRITICAL")throw new Error(); const observedPositions=Number(value.observedPositions),availableLiveMarks=Number(value.availableLiveMarks);if(!Number.isSafeInteger(observedPositions)||observedPositions<0||!Number.isSafeInteger(availableLiveMarks)||availableLiveMarks<0)throw new Error();resolve({observedPositions,availableLiveMarks,monitoringTier:tier,numericStopObserved:value.numericStopObserved===true,emergencyStopObserved:value.emergencyStopObserved===true,outputBytes:combined.length});}catch{reject(new Error('LPFORGE_P7_POSITION_HEALTH_PROBE_OUTPUT_INVALID'));}}));});
}
/**
 * Recovery blocks new exposure, never an independently revalidated terminal
 * protective close.  The child command has no signing capability; it can only
 * persist a canonical CLOSE/EMERGENCY_CLOSE plan for the execution worker.
 */
export async function runAutonomousProtectiveManagementProbe(input:{cwd:string;env:NodeJS.ProcessEnv}):Promise<Phase7OperatorProbe>{
  const childEnv:{[key:string]:string|undefined}={...input.env,LIVE_SIGNING:'false',LPFORGE_LIVE_EXECUTION:'false',LPFORGE_MAINNET_CANARY:'false',LPFORGE_DATA_MODE:'LIVE_READ_ONLY',LPFORGE_OPERATOR_MACHINE_SUMMARY:'true'};
  delete childEnv.LPFORGE_PREPARE_POSITION_ADDRESS;
  delete childEnv.LPFORGE_P6_PRIVATE_KEY;delete childEnv.LPFORGE_P6_PRIVATE_WRITE_RPC_URL;
  delete childEnv.LPFORGE_P6_SIGNER_BACKEND_ID;delete childEnv.LPFORGE_P6_SIGNER_MODE;delete childEnv.LPFORGE_P6_SIGNER_PUBLIC_KEY;
  delete childEnv.LIVE_SUBMISSION;delete childEnv.LPFORGE_P6_EXECUTION_RUNNER_ENABLED;
  const timeoutMs=Math.max(30_000,Math.min(300_000,Number(input.env.LPFORGE_P7_PROTECTIVE_MANAGEMENT_TIMEOUT_MS??60_000)));
  return new Promise((resolve,reject)=>{const child=spawn('node',['--enable-source-maps','.build/apps/operator/src/main.js','manage-owned-positions-protective'],{cwd:input.cwd,env:childEnv,stdio:['ignore','pipe','pipe']});let out='',err='',settled=false;const timeout=setTimeout(()=>{if(settled)return;settled=true;(child as unknown as {kill:(signal:string)=>boolean}).kill('SIGTERM');reject(new Error(`LPFORGE_P7_PROTECTIVE_MANAGEMENT_TIMEOUT:${timeoutMs}:${redactProbeFailureDetail(err||out)}`));},timeoutMs);const done=(fn:()=>void)=>{if(settled)return;settled=true;clearTimeout(timeout);fn();};child.stdout?.on('data',x=>{out+=String(x);});child.stderr?.on('data',x=>{err+=String(x);});child.on('error',error=>done(()=>reject(error)));child.on('close',code=>done(()=>{const combined=out+'\n'+err;const exitCode=code??1;if(exitCode!==0)return reject(new Error(`LPFORGE_P7_PROTECTIVE_MANAGEMENT_FAILED:${exitCode}:${redactProbeFailureDetail(err||out)}`));resolve(parsePhase7OperatorProbeOutput(combined,exitCode));}));});
}
/**
 * Recovery blocks new exposure, never the P7 health/control observations
 * themselves. The control plane must keep publishing fresh facts while P6
 * finishes earlier work; otherwise a healthy RPC is eventually displayed as
 * UNKNOWN and a stale control row can be mistaken for current authority.
 */
export interface FundedOpenContinuationControlFact {planId:string;ownerAddress:string;poolAddress:string;tokenMint:string;fundingSignature:string;capitalLamports:string;lowerBinId:number;upperBinId:number;fundedAt:string;expiresAt:string;generatedPositionAddress?:string;allowed:boolean;reasonCodes:string[];}
function continuationRecord(value:unknown):Record<string,unknown>{return value&&typeof value==='object'&&!Array.isArray(value)?value as Record<string,unknown>:{};}
/** A confirmed funded row is a distinct, exact-plan completion lane. */
export function deriveFundedOpenContinuationControlFacts(input:{rows:ReadonlyArray<Record<string,unknown>>;now:string;deadlineSeconds:number;baseControlAllowsContinuation:boolean;recoveryQueueCount:number;unknownSubmissionCount:number;unresolvedReconciliationDebt:number;partialEntryRecoveryCount:number}):FundedOpenContinuationControlFact[]{
 const nowMs=Date.parse(input.now);
 return input.rows.map(row=>{const payload=continuationRecord(row.payload),saved=continuationRecord(payload.fundedOpenContinuation),fundedAt=new Date(String(row.funded_at??'')).toISOString(),expiresAt=new Date(Date.parse(fundedAt)+input.deadlineSeconds*1_000).toISOString(),range=continuationRecord(row.intended_range),lowerBinId=Number(range.lowerBinId),upperBinId=Number(range.upperBinId),otherFundedContinuation=input.rows.some(candidate=>String(candidate.plan_id)!==String(row.plan_id)),unrelatedDebt=input.recoveryQueueCount>0||input.unknownSubmissionCount>0||input.unresolvedReconciliationDebt>0||input.partialEntryRecoveryCount>0||otherFundedContinuation,reasonCodes:string[]=[];
   if(row.funding_confirmed!==true)reasonCodes.push('P7_FUNDED_OPEN_FUNDING_NOT_CONFIRMED');if(row.open_submission_unresolved===true)reasonCodes.push('P7_FUNDED_OPEN_SUBMISSION_UNRESOLVED');if(row.position_open===true)reasonCodes.push('P7_FUNDED_OPEN_POSITION_ALREADY_OPEN');if(!['CLAIMED','SIGNING','SIGNED','SUBMITTED','RECONCILIATION_REQUIRED','RECOVERING'].includes(String(row.plan_state??'')))reasonCodes.push('P7_FUNDED_OPEN_PLAN_STATE_INVALID');if(!Number.isFinite(Date.parse(fundedAt))||!Number.isFinite(nowMs)||Date.parse(expiresAt)<=nowMs)reasonCodes.push('P7_FUNDED_OPEN_CONTINUATION_EXPIRED');if(!Number.isInteger(lowerBinId)||!Number.isInteger(upperBinId)||lowerBinId>upperBinId)reasonCodes.push('P7_FUNDED_OPEN_RANGE_INVALID');if(unrelatedDebt)reasonCodes.push('P7_FUNDED_OPEN_UNRELATED_RECOVERY_PENDING');if(!input.baseControlAllowsContinuation)reasonCodes.push('P7_FUNDED_OPEN_CONTROL_NOT_ELIGIBLE');
   const generatedPositionAddress=typeof saved.generatedPositionAddress==='string'&&saved.generatedPositionAddress.trim()?saved.generatedPositionAddress.trim():undefined;
   return{planId:String(row.plan_id),ownerAddress:String(row.owner_address),poolAddress:String(row.pool_address),tokenMint:String(row.token_mint),fundingSignature:String(row.funding_signature),capitalLamports:String(row.intended_capital_lamports),lowerBinId,upperBinId,fundedAt,expiresAt,...(generatedPositionAddress?{generatedPositionAddress}:{}),allowed:reasonCodes.length===0,reasonCodes:reasonCodes.sort()};
 }).sort((a,b)=>a.planId.localeCompare(b.planId));
}
export function phase7RecoveryBlocksNewEntries(recovery:{recoveryQueueCount:number;unknownSubmissionCount:number;unresolvedReconciliationDebt:number;partialEntryRecoveryCount:number;fundedOpenContinuations?:ReadonlyArray<Record<string,unknown>>}):boolean{return recovery.recoveryQueueCount>0||recovery.unknownSubmissionCount>0||recovery.unresolvedReconciliationDebt>0||recovery.partialEntryRecoveryCount>0||(recovery.fundedOpenContinuations?.length??0)>0;}
export async function runPhase7ProductionOnce(input:{cfg:Phase1Config;store:Phase1Store;runtimeId:string;instanceId:string;cycleKey:string;driftBaseline:Phase7EvaluationMetrics;sourceCommit?:string;policyHash?:string;cwd:string;env:NodeJS.ProcessEnv;leaseTtlMs?:number;restarted:boolean}):Promise<Phase7ProductionOnceResult>{
  ...
  const recovery=await input.store.loadPhase7RecoveryFacts(input.runtimeId),recoveryBlocksNewEntries=phase7RecoveryBlocksNewEntries(recovery),executionPolicy=loadDeploymentPolicyFile(resolveLiveExecutionPolicyPath(input.env)),continuationDeadlineSeconds=executionPolicy.fundedOpenContinuation?.deadlineSeconds;
  if(continuationDeadlineSeconds===undefined||!Number.isInteger(continuationDeadlineSeconds))throw new Error('LPFORGE_P7_FUNDED_OPEN_CONTINUATION_POLICY_REQUIRED');
