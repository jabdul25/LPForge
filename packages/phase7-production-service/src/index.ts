...
export async function runAutonomousDecisionProbe(input:{cwd:string;env:NodeJS.ProcessEnv;poolAddress?:string}):Promise<Phase7OperatorProbe>{
  ...
  return new Promise((resolve,reject)=>{const child=spawn('node',['--enable-source-maps','.build/apps/operator/src/main.js','live-once'],{cwd:input.cwd,env:childEnv,stdio:['ignore','pipe','pipe']});let out='',err='';child.stdout?.on('data',x=>{out+=String(x);});child.stderr?.on('data',x=>{err+=String(x);});child.on('error',reject);child.on('close',code=>{const combined=out+'\n'+err;const exitCode=code??1;if(exitCode!==0)return reject(new Error(`LPFORGE_P7_OPERATOR_PROBE_FAILED:${exitCode}:${redactProbeFailureDetail(err||out)}`));resolve(parsePhase7OperatorProbeOutput(combined,exitCode,poolAddress));});});
}
