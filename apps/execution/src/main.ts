...
import { alertsForExecutionRecovery,alertsForExecutionResult,enqueueAndDispatchPhase7Alert,loadPhase7TelegramConfig,RpcQuotaAlertObserver,type Phase7Alert } from "../../../packages/phase7-alerting/src/index.js";
...
function phase7ExecutionControlFromRow(controlRow:Record<string,unknown>|undefined):Phase7ExecutionControl|undefined{
  ...
  return{decisionId:String(controlRow.decision_id),cycleKey:String(controlRow.cycle_key),authorityMode:String(controlRow.authority_mode),healthStatus:String(controlRow.health_status),driftStatus:String(controlRow.drift_status),safetyMode:String(controlRow.safety_mode),newEconomicActionAllowed:Boolean(controlRow.new_economic_action_allowed),observedAt:new Date(String(controlRow.observed_at)).toISOString(),poolDrift:Object.fromEntries(rawPoolDrift.filter(row=>row&&typeof row==='object').map(row=>{const value=row as Record<string,unknown>;return[String(value.poolAddress??''),String(value.rawStatus??value.status??'')]}).filter(([pool])=>Boolean(pool))),blockedPools:stringArray(payload.blockedPools),activeIncidentIds:stringArray(payload.activeIncidentIds),releaseIntegrityValid:releaseIdentity.valid===true,portfolioValid:portfolio.valid===true,revokedApprovalIds:stringArray(payload.controlledCanaryRevokedApprovalIds)};
}
const executionTelegramConfig=loadPhase7TelegramConfig({...process.env,LPFORGE_TELEGRAM_MIN_SEVERITY:process.env.LPFORGE_EXECUTION_TELEGRAM_MIN_SEVERITY??process.env.LPFORGE_TELEGRAM_MIN_SEVERITY??'INFO'});
const executionRpcQuotaObserver=new RpcQuotaAlertObserver();
async function safeExecutionTelegramAlert(alert:Phase7Alert){try{const r=await enqueueAndDispatchPhase7Alert({databaseUrl:process.env.DATABASE_URL,alert,config:executionTelegramConfig});if(r.sent)console.log(json({event:'lpforge_telegram_alert_sent',runtime:'lpforge-execution',code:alert.code,severity:alert.severity,durable:true}));}catch(error){console.error(json({event:'lpforge_telegram_alert_failed',runtime:'lpforge-execution',code:alert.code,error:error instanceof Error?error.message:String(error)}));}}
...
async function alertExecutionRecovery(recovery:{partial:Array<{planId:string;reasonCodes:string[]}>;plans:Array<{planId:string;reasonCodes:string[]}>;walletSweep:{reasonCodes:string[]}},observedAt:string){for(const alert of alertsForExecutionRecovery({observedAt,runtimeId:'lpforge-execution',partial:recovery.partial,plans:recovery.plans,walletReasonCodes:recovery.walletSweep.reasonCodes}))await safeExecutionTelegramAlert(alert);}
async function observeExecutionRpcQuotaFromRuntime(successfulRead:boolean,observedAt:string){if(!executionRpcQuotaObserver.due())return;const store=await createPostgresStore(process.env.DATABASE_URL??'');try{const config=recoveryConfig(),providerKey=rpcProviderKey(config.rpcUrl),state=await store.loadRpcProviderBudgetState(providerKey);for(const alert of executionRpcQuotaObserver.alerts({lane:'EXECUTION',runtimeId:'lpforge-execution',providerKey,observedAt,successfulRead,...(state?{state}:{})}))await safeExecutionTelegramAlert(alert);}catch(error){console.error(json({event:'lpforge_rpc_quota_alert_observer_failed',runtime:'lpforge-execution',error:error instanceof Error?error.message:String(error)}));}finally{await store.close();}}
...
async function dispatchOne(options?: { protectiveOnly?: boolean }) {
  ...
  try {
    ...
    if(["OPEN","ADD"].includes(plan.action)){
      ...
      const reserved=await store.reserveExecutionCapital({planId:plan.planId,ownerAddress:plan.ownerAddress,poolAddress:plan.poolAddress,capitalLamports:capital,walletLamports,reserveLamports:deployment.reserveLamports,maxPortfolioLamports:deployment.maxPortfolioLamports,maxPoolLamports:poolCap,maxTokenLamports:deployment.maxTokenLamports,minInitialPositionLamports:deployment.minInitialPositionLamports,maxInitialPositionLamports:deployment.maxInitialPositionLamports,now});
...
async function start() {
  if (!yes(process.env.LPFORGE_P6_EXECUTION_RUNNER_ENABLED)) {
    ...
    if (executionTelegramConfig.notifyStartup)
    ...
    for (;;) {
      ...
      try { console.log(json({...await reconcileOnlyOnce(),observedAt})); await observeExecutionRpcQuotaFromRuntime(true,observedAt); }
      catch (error) { console.error(error); await safeExecutionTelegramAlert({severity:'CRITICAL',code:'P6_EXECUTION_RECONCILE_ONLY_EXCEPTION',title:'Recovery check needs a safe retry',message:'LPForge could not complete this recovery check yet. It will retry safely and will not send a duplicate transaction.\nAction needed: none right now.',runtimeId:'lpforge-execution',observedAt,reasonCodes:['P6_EXECUTION_RECONCILE_ONLY_EXCEPTION']}); }
