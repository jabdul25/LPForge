...
async function observeAndPlanOwnedPositions(input: {
  ...
  for (const row of positions) {
    ...
    if(priorLifecycleState!==oor.state){
      ...
      queueLifecycleAlert({...alertBase,severity:reentered?'INFO':'WARNING',code,title:reentered?'Position range re-entered':oor.state==='TRANSIENT_OOR'?'Position out of range':'Position OOR escalation',message:reentered?'The position returned to its configured range.':'Canonical range lifecycle changed; no action was inferred by Telegram.',transitionKey:`${priorLifecycleState??'UNOBSERVED'}->${oor.state}`,reasonCodes:oor.reasonCodes,details:{Direction:oor.direction??'N/A','Lifecycle':oor.state,'Action':oor.action,'OOR seconds':oor.continuousOorDurationSeconds,'Active bin':activeBinId,'Range':`${position.lowerBinId} → ${position.upperBinId}`,'Inventory':oor.inventoryClassification}});
    }
