...
import { enqueueAndDispatchPhase7Alert, loadPhase7TelegramConfig, type Phase7Alert } from "../../../packages/phase7-alerting/src/index.js";
...
async function observeAndPlanOwnedPositions(input: {
  ...
  for (const row of positions) {
    ...
    if(priorLifecycleState!==oor.state){
      const reentered=oor.state==='IN_RANGE'&&priorLifecycleState&&priorLifecycleState!=='IN_RANGE';
