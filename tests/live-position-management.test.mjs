...
test('unavailable chain truth or active management plan never advances or dispatches the below close',()=>{
  ...
  const unreconciled=oor('2026-08-13T00:30:00Z',prior(first),'SAFE_OOR_SOL','OUT_OF_RANGE',80,{reconciliationClean:false});assert.equal(unreconciled.action,'HOLD_CHAIN_RECONCILIATION');assert.ok(unreconciled.reasonCodes.includes('POSITION_OOR_RECONCILIATION_REQUIRED'));
  ...
  const close=oor('2026-08-13T00:30:00Z',prior(first),'SAFE_OOR_SOL','OUT_OF_RANGE',80);assert.equal(close.action,'CLOSE_AND_REEVALUATE');
  const duplicate=oor('2026-08-13T00:30:00Z',prior(close),'SAFE_OOR_SOL','OUT_OF_RANGE',80,{noActiveManagementPlan:false});assert.equal(duplicate.action,'HOLD_CHAIN_RECONCILIATION');assert.ok(duplicate.reasonCodes.includes('POSITION_OOR_MANAGEMENT_PLAN_PENDING'));
});
...
test('only explicitly position-bound protective management may proceed without a matching pool context',()=>{
  ...
  const profitProtection=assessLiveManagementContext({positionPoolAddress:'POOL_B',managementPoolAddress:'POOL_A',action:'CLOSE',profitProtectionClose:true});
