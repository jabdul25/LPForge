...
test('claim guard rejects forged provenance, non-admitted pools, excess capital and policy-defined global position limits',()=>{const input={policy,ownedPositions:[],productionCandidates:admittedStatic,phase7Control:control,now};assert.equal(validateClaimedPlan({plan:base,...input}).approved,true);assert.ok(validateClaimedPlan({plan:{...base,planPayload:{}},...input}).reasonCodes.includes('P6_CLAIM_PROVENANCE_INVALID'));assert.ok(validateClaimedPlan({plan:{...base,poolAddress:'OTHER'},...input}).reasonCodes.includes('P6_CLAIM_PRODUCTION_ADMISSION_INVALID'));assert.ok(validateClaimedPlan({plan:base,policy,ownedPositions:[],productionCandidates:[],phase7Control:control,now}).reasonCodes.includes('P6_CLAIM_PRODUCTION_ADMISSION_INVALID'),'static membership alone cannot admit a new entry');assert.ok(validateClaimedPlan({plan:{...base,planPayload:{...base.planPayload,intent:{capitalLamports:'20000001'}}},...input}).reasonCodes.includes('P6_CLAIM_CAPITAL_EXCEEDS_POOL_POLICY'));assert.ok(validateClaimedPlan({plan:base,policy,ownedPositions:[{lifecycle_state:'OPEN',pool_address:'OTHER',owner_address:'OWNER'}],productionCandidates:admittedStatic,phase7Control:control,now}).approved,'Pool B may coexist with Pool A under max=2');assert.ok(validateClaimedPlan({plan:base,policy,ownedPositions:[{lifecycle_state:'OPEN',pool_address:'POOL',owner_address:'OWNER'}],productionCandidates:admittedStatic,phase7Control:control,now}).reasonCodes.includes('P6_CLAIM_LIVE_POSITION_ALREADY_EXISTS_FOR_POOL'));assert.ok(validateClaimedPlan({plan:base,policy,ownedPositions:[{lifecycle_state:'OPEN',pool_address:'OTHER',owner_address:'OWNER'},{lifecycle_state:'OPEN',pool_address:'THIRD',owner_address:'OWNER'}],productionCandidates:admittedStatic,phase7Control:control,now}).reasonCodes.includes('P6_CLAIM_POSITION_LIMIT'));});
test('same-pool ownership is scoped to the owner rather than a global pool singleton',()=>{const result=validateClaimedPlan({plan:base,policy,ownedPositions:[{lifecycle_state:'OPEN',pool_address:'POOL',owner_address:'OTHER_OWNER'}],productionCandidates:admittedStatic,phase7Control:control,now});assert.equal(result.approved,true);});
test('Telegram pool blacklist blocks only new risk-increasing entry and never substitutes for protective close authority',()=>{const blocked=validateClaimedPlan({plan:base,policy,ownedPositions:[],productionCandidates:admittedStatic,phase7Control:{...control,blockedPools:['POOL']},now});assert.ok(blocked.reasonCodes.includes('P6_CLAIM_OPERATOR_POOL_BLACKLISTED'));const close={...base,action:'CLOSE',positionAddress:'POS',planPayload:{...base.planPayload,intent:{capitalLamports:'0'}}};const protective=validateClaimedPlan({plan:close,policy,ownedPositions:[{lifecycle_state:'OPEN',position_address:'POS',owner_address:'OWNER',pool_address:'POOL'}],positionTruth:{owner:'OWNER',pool:'POOL'},phase7Control:{...control,blockedPools:['POOL']},now});assert.equal(protective.approved,true);});
test('P6 rejects an invalid global position policy before capacity comparison',()=>{const result=validateClaimedPlan({plan:base,policy:{...policy,maxOpenPositions:Number.NaN},ownedPositions:[],productionCandidates:admittedStatic,phase7Control:control,now});assert.ok(result.reasonCodes.includes('P6_CLAIM_POSITION_LIMIT_INVALID'));});
...
test('a fresh authenticated Tier-A winner remains P6-admissible across Tier-A to Tier-B rank drift',()=>{
...
test('recovery dispatch may claim only terminal protective plans',async()=>{const db=await import('node:fs/promises').then(fs=>fs.readFile('packages/db/src/index.ts','utf8')),execution=await import('node:fs/promises').then(fs=>fs.readFile('apps/execution/src/main.ts','utf8'));assert.match(db,/\$2::boolean=false OR i\.action IN \('CLOSE','EMERGENCY_CLOSE'\)/);assert.match(execution,/dispatchOne\(\{protectiveOnly:true\}\)/);assert.match(execution,/claimNextAutonomousPlan\(new Date\(\)\.toISOString\(\), options\)/);});

test('P6 candidate freshness uses the current collector observation rather than stale deep-screen metadata',async()=>{
 const db=await import('node:fs/promises').then(fs=>fs.readFile('packages/db/src/index.ts','utf8'));
 assert.match(db,/last_seen_at AS admission_seen_at/);
 assert.doesNotMatch(db,/COALESCE\(NULLIF\(payload->>'deepScreenedAt',''\)::timestamptz,last_seen_at\) AS admission_seen_at/);
});
