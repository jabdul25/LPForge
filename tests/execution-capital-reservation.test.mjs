...
const request={planId:'plan',ownerAddress:'owner',poolAddress:'pool',capitalLamports:10n*L,walletLamports:30n*L,reserveLamports:1n*L,maxPortfolioLamports:50n*L,maxPoolLamports:50n*L,maxTokenLamports:50n*L,minInitialPositionLamports:10n*L,maxInitialPositionLamports:50n*L,now:'2026-08-16T00:00:00.000Z'};
...
test('final reservation enforces the policy minimum and supports non-0.03 policy envelopes',()=>{
  assert.ok(assessExecutionCapitalReservation({request:{...request,capitalLamports:9n*L},...facts}).reasonCodes.includes('P6_CAPITAL_MIN_INITIAL_POSITION'));
  for(const capital of [20_000_000n,30_000_000n,40_000_000n]){
    const result=assessExecutionCapitalReservation({request:{...request,capitalLamports:capital,minInitialPositionLamports:capital,maxInitialPositionLamports:capital,walletLamports:1_000_000_000n,maxPortfolioLamports:2_000_000_000n,maxPoolLamports:2_000_000_000n,maxTokenLamports:2_000_000_000n},...facts,deployedLamports:0n,poolDeployedLamports:0n,tokenDeployedLamports:0n});
    assert.equal(result.approved,true,`${capital} must propagate without a fixed-size clamp`);
  }
});
