...
test('a stale non-serviceable timing sample admits exactly one bounded remeasurement lease',async()=>{
 const candidates=[{poolAddress:'RECOVERY',state:'QUALIFIED',tier:'A',priorityScore:1,lastSeenAt:at,payload:{}}],admissions=[];
 const store={loadActiveCandidateEvidenceCollectorTiming:async()=>({p95PoolCollectionMs:370_850,observedAt:'2026-08-12T15:00:00.000Z'}),reconcileLiveEvidenceAdmission:async x=>{admissions.push(x);candidates[0].state='ACTIVE_CANDIDATE';return{};},listDiscoveryCandidates:async()=>candidates,insertPoolSnapshot:async()=>{},insertBins:async()=>{},insertDataApiPool:async()=>{},insertOhlcv:async()=>{},insertFeeVolumeObservations:async()=>{},loadFeeVolumeObservations:async()=>feeBuckets(48).map(x=>({bucketAt:new Date(x.timestamp*1000).toISOString(),fees:x.fees,protocolFees:x.protocol_fees,volume:x.volume})),insertCandidateMarketObservations:async()=>{},loadCandidateMarketObservations:async()=>[{observedAt:at,sourceType:'HISTORICAL_API_BACKFILL',sourceProvider:'test',price:1,resolutionMs:300000}],loadActiveCandidateBackfill:async()=>({last_successful_at:at}),upsertActiveCandidateBackfill:async()=>{},insertSwapEvent:async()=>{},loadOperationalHistory:async()=>history(61),upsertActiveCandidateHistoryMaturity:async()=>{},insertEconomicEstimate:async()=>{},recordLiveEvidenceCollectionOutcome:async()=>{},recordActiveCandidateEvidenceCollectorPass:async()=>{}};
 const api={getPool:async address=>({address,tvl:100000}),getHistoricalVolume:async()=>({data:feeBuckets(48)}),getOhlcv:async()=>({data:[]})},adapter={getPool:async address=>({address,activeBinId:1}),getBinsAroundActive:async()=>[],decodeEvents:async()=>[]},rpc={getSignaturesForAddress:async()=>[],getTransaction:async()=>null};
 const result=await collectActiveCandidateEvidence({api,adapter,rpc,store,observedAt:at,policy:{maxConcurrentPoolReads:2,p95PoolCollectionMs:90_000,liveConfirmationTargetCoverageMs:180_000}});
 assert.equal(admissions[0].serviceableCapacity,1);assert.equal(result.serviceableCapacity,1);assert.equal(result.results.filter(x=>x.collectionTarget==='ACTIVE_ECONOMIC').length,1);
});
test('continuity tracking preserves full-frame collection through cooldown without becoming a third economic lease',async()=>{
 ...
 assert.equal(result.serviceableCapacity,2);assert.equal(result.economicCollectionSliceSize,2);assert.equal(result.continuityCollectionSliceSize,1);assert.equal(result.results.filter(x=>x.collectionTarget==='ACTIVE_ECONOMIC').length,2);assert.equal(result.results.filter(x=>x.collectionTarget==='EVIDENCE_CONTINUITY').length,1);assert.deepEqual(result.results.map(x=>x.collectionTarget),['ACTIVE_ECONOMIC','ACTIVE_ECONOMIC','EVIDENCE_CONTINUITY'],'active leases dispatch before auxiliary continuity work');assert.equal(live.length,2);assert.equal(continuity.length,1);assert.equal(result.capacityViolation,false);
 const activeOnly=await collectActiveCandidateEvidence({api,adapter,rpc,store,observedAt:new Date().toISOString(),scope:'ACTIVE',policy:{maxConcurrentPoolReads:1,p95PoolCollectionMs:90_000,liveConfirmationTargetCoverageMs:180_000,liveConfirmationMaxGapMs:450_000}}),auxiliaryOnly=await collectActiveCandidateEvidence({api,adapter,rpc,store,observedAt:new Date().toISOString(),scope:'AUXILIARY',policy:{maxConcurrentPoolReads:1,p95PoolCollectionMs:90_000,liveConfirmationTargetCoverageMs:180_000,liveConfirmationMaxGapMs:450_000}});
 assert.deepEqual(activeOnly.results.map(x=>x.collectionTarget),['ACTIVE_ECONOMIC','ACTIVE_ECONOMIC']);assert.deepEqual(auxiliaryOnly.results.map(x=>x.collectionTarget),['EVIDENCE_CONTINUITY']);
});
