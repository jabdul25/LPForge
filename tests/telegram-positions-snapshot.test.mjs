...
test('telegram operator uses the bounded DB summary reader for /positions',async()=>{
  ...
  assert.match(db,/incident_id IN \('telegram:pause','telegram:stop'\)/,'/resume resolves the reserved Telegram control identities');
  assert.doesNotMatch(db,/payload->>'telegramOperator'='true' AND incident_id IN \('telegram:pause','telegram:stop'\)/,'/resume remains compatible with valid legacy Telegram controls that predate the payload marker');
