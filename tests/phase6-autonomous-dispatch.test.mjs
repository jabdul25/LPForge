...
test('P6 autonomous dispatcher maps an extended position and every SDK chunk to durable plan steps',async()=>{
  ...
  const prepared=await prepareAutonomousMeteoraOpen({plan,pool,liquiditySlippageBps:100});assert.deepEqual(prepared.steps.map(step=>step.transactionId),['tx-extend','tx-chunk-1','tx-chunk-2']);assert.deepEqual(prepared.steps.map(step=>step.kind),['METEORA_POSITION_EXTEND','METEORA_OPEN_CHUNK','METEORA_OPEN_CHUNK']);assert.ok(prepared.steps[0].requiredSignerAddresses.includes(prepared.positionSigner.publicKeyAddress));assert.deepEqual(auxiliaryPositionSignersForOpenStep(prepared.steps[0],prepared.positionSigner),[prepared.positionSigner]);assert.deepEqual(auxiliaryPositionSignersForOpenStep(prepared.steps[1],prepared.positionSigner),[]);assert.deepEqual(auxiliaryPositionSignersForOpenStep(prepared.steps[2],prepared.positionSigner),[]);
});
test('P6 autonomous dispatcher fails closed when an OPEN plan lacks its policy-bound maximum width',async()=>{
  const owner=Keypair.generate().publicKey.toBase58(),plan={planId:'plan-missing-width',intentId:'intent-missing-width',idempotencyKey:'key-missing-width',poolAddress:'pool-1',ownerAddress:owner,thesisId:'thesis-1',observedAt:'2026-08-13T00:00:00Z',expiresAt:'2030-08-13T00:05:00Z',intentPayload:{entryFunding:{totalPairedTokenRaw:'0',solForLpLamports:'20000000'}},planPayload:{intent:{lowerBinId:1,upperBinId:2,strategy:'BID_ASK'}},transactionId:'tx-missing-width',transactionMetadata:{}};
  await assert.rejects(()=>prepareAutonomousMeteoraOpen({plan,pool:{},liquiditySlippageBps:100}),/LPFORGE_P6_AUTONOMOUS_PLAN_MAX_POSITION_WIDTH/);
});
