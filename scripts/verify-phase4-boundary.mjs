...
for(const file of files){const rel=file.replaceAll('\\','/');if(allowedMentions.has(rel))continue;const text=fs.readFileSync(file,'utf8');if(text.includes('LPFORGE_PHASE5_EXECUTION_MODULE'))continue;for(const rule of forbidden)if(rule.test(text)){console.error('PHASE4_BOUNDARY_FAIL',rel,rule);process.exit(1);}}
