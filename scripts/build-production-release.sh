...
node -e 'const fs=require("fs");fs.writeFileSync(process.argv[1],JSON.stringify({generatedForCommit:process.argv[2],included:["tracked source selected by git archive","generated RELEASE_MANIFEST.json","generated SOURCE_REVISION.txt","generated SHA256SUMS.txt"],excluded:[".env",".env.* except .env.example","private keys","wallet/keypair files","API/RPC credentials","node_modules",".pnpm-store","runtime databases","SOURCE_GIT.bundle"]},null,2)+"\n")' "$stage/SECURITY_SANITIZATION.json" "$sha"
(
  cd "$stage"
