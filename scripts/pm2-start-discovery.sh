...
node -e 'const fs=require("node:fs");const p=process.env.LPFORGE_DISCOVERY_POLICY_PATH||"release-policy-templates/pool-discovery-policy.json";if(!fs.existsSync(p))process.exit(2);const x=JSON.parse(fs.readFileSync(p,"utf8"));if(x.schemaVersion!==1)process.exit(2);'
