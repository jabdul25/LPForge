...
const violations=[];
for(const root of roots)for(const file of walk(root).filter((x)=>x.endsWith('.ts'))){const text=fs.readFileSync(file,'utf8');if(text.includes('LPFORGE_PHASE5_EXECUTION_MODULE')||text.includes('LPFORGE_PHASE6_MAINNET_MODULE'))continue;for(const re of prohibited)if(re.test(text))violations.push(`${file}: ${re}`);}
